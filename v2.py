"""
AutoML Pipeline for Tabular Data (v2.0 — Dual Mode)
=====================================================
A universal machine learning pipeline that automatically:
  ...
  4. Detects the task type (classification / regression / both)
  5. Trains & compares multiple models
  6. Evaluates and generates a summary report

NEW in v2: --task both  runs classification AND regression simultaneously.
  - For a continuous target: regression runs normally, and the target is
    auto-binned into classes (quantile-based) for a parallel classification run.
  - For a categorical target: classification runs normally, and the target is
    label-encoded for a parallel regression run.
  - A combined comparison report shows results side by side.

Usage:
    python auto_ml_pipeline.py --data your_data.csv --target target_column
    python auto_ml_pipeline.py --data your_data.csv --target target_column --task both
...
class PipelineConfig:
    ...
    n_bins_for_classification: int = 4  # bins when converting continuous → classes
...
def preprocess(
    ...
    Returns: X, y, preprocessor, label_encoder
    """
    df = df.copy()

    # ── Drop columns that are mostly missing ──
    drop_cols = [
        c for c, pct in profile["missing_pct"].items()
...
# ─────────────────────────────────────────────────────────────────────────────
# 5. Target Conversion for Dual Mode
# ─────────────────────────────────────────────────────────────────────────────

def bin_continuous_target(y: pd.Series, n_bins: int = 4) -> tuple:
    """
    Convert a continuous target into quantile-based bins for classification.
    Returns (y_binned, bin_labels, bin_edges).
    """
    try:
    ...
    for i in range(actual_bins):
        low = bin_edges[i]
        high = bin_edges[i + 1]
        bin_labels.append(f"Bin{i} [{low:.1f}-{high:.1f}]")

    print(f"  Binned continuous target into {actual_bins} classes (quantile-based):")
...
def encode_categorical_as_numeric(y: pd.Series, label_encoder: LabelEncoder) -> pd.Series:
    """
    Convert an already label-encoded categorical target into numeric values
    for regression (just uses the integer codes).
    """
    print(f"  Using label-encoded target as numeric for regression (codes 0-{y.max()})")
...
def evaluate_best_model(
    ...
    print("\n" + "=" * 60)
    print(f"  HOLDOUT TEST EVALUATION — {task.upper()}{tag}")
    print("=" * 60)
    ...
    if task == "classification":
        ...
        plt.tight_layout()
        fname = f"confusion_matrix{file_suffix}.png"
        ...
        ax.legend()
        plt.tight_layout()
        fname = f"actual_vs_predicted{file_suffix}.png"
        plt.savefig(os.path.join(output_dir, fname), dpi=150)
        plt.close()
        print(f"  → Saved {fname}")
...
def save_feature_importance(pipeline, X, output_dir, suffix=""):
    ...
    try:
        ...
        if hasattr(model, "feature_importances_"):
            ...
            plt.tight_layout()
            fname = f"feature_importance{file_suffix}.png"
            plt.savefig(os.path.join(output_dir, fname), dpi=150)
            plt.close()
            print(f"  → Saved {fname}")

            imp_df.to_csv(os.path.join(output_dir, f"feature_importance{file_suffix}.csv"), index=False)
    except Exception:
        pass


def save_model_comparison_chart(results_df, best_name, task, output_dir, suffix=""):
    """Save a horizontal bar chart comparing model scores."""
    file_suffix = f"_{suffix}" if suffix else ""
    fig, ax = plt.subplots(figsize=(10, 6))
    metric_label = "F1-Weighted" if task == "classification" else "RMSE"
    results_sorted = results_df.sort_values("Display_Score", ascending=True)
    colors = ["#2196F3" if m != best_name else "#FF9800" for m in results_sorted["Model"]]
    ax.barh(results_sorted["Model"], results_sorted["Display_Score"], color=colors)
    ax.set_xlabel(metric_label)
    tag = f" ({suffix})" if suffix else ""
    ax.set_title(f"Model Comparison — {metric_label}{tag}")
    plt.tight_layout()
    fname = f"model_comparison{file_suffix}.png"
    plt.savefig(os.path.join(output_dir, fname), dpi=150)
    plt.close()
    print(f"  → Saved {fname}")
...
def generate_dual_report(
    ...
    """Generate a side-by-side comparison report for dual mode."""
    print("\n" + "=" * 60)
    print("  DUAL-MODE COMBINED REPORT")
    print("=" * 60)

    print(f"\n  {'':30} {'CLASSIFICATION':>18} {'REGRESSION':>18}")
    print("  " + "-" * 66)
    print(f"  {'Best Model':<30} {clf_best:>18} {reg_best:>18}")
    print(f"  {'Best CV F1 / RMSE':<30} {clf_results['Display_Score'].max():>18.4f} {reg_results['Display_Score'].min():>18.4f}")
    print()
    print(f"  {'--- Holdout Metrics ---':<30}")

    if clf_metrics:
        print(f"  {'Accuracy':<30} {clf_metrics['accuracy']:>18.4f} {'--':>18}")
        print(f"  {'F1-Score':<30} {clf_metrics['f1']:>18.4f} {'--':>18}")
        print(f"  {'Precision':<30} {clf_metrics['precision']:>18.4f} {'--':>18}")
        print(f"  {'Recall':<30} {clf_metrics['recall']:>18.4f} {'--':>18}")
    if reg_metrics:
        print(f"  {'RMSE':<30} {'--':>18} {reg_metrics['rmse']:>18.4f}")
        print(f"  {'MAE':<30} {'--':>18} {reg_metrics['mae']:>18.4f}")
        print(f"  {'R-squared':<30} {'--':>18} {reg_metrics['r2']:>18.4f}")
    ...
    clf_sorted = clf_results.sort_values("Display_Score", ascending=True)
    colors1 = ["#2196F3" if m != clf_best else "#FF9800" for m in clf_sorted["Model"]]
    ax1.barh(clf_sorted["Model"], clf_sorted["Display_Score"], color=colors1)
    ax1.set_xlabel("F1-Weighted")
    ax1.set_title("Classification Models")
    ax1.set_xlim(0, 1.05)

    reg_sorted = reg_results.sort_values("Display_Score", ascending=True)
    colors2 = ["#4CAF50" if m != reg_best else "#FF9800" for m in reg_sorted["Model"]]
    ax2.barh(reg_sorted["Model"], reg_sorted["Display_Score"], color=colors2)
    ax2.set_xlabel("RMSE")
    ax2.set_title("Regression Models")

    plt.suptitle("Dual-Mode Model Comparison", fontsize=14, fontweight="bold", y=1.02)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "dual_mode_comparison.png"), dpi=150, bbox_inches="tight")
    plt.close()
    print(f"\n  → Saved dual_mode_comparison.png")

    # Save combined CSV
    combined = pd.concat([clf_results.copy(), reg_results.copy()], ignore_index=True)
    combined.to_csv(os.path.join(output_dir, "dual_mode_all_models.csv"), index=False)
    print(f"  → Saved dual_mode_all_models.csv")


# ─────────────────────────────────────────────────────────────────────────────
# 9. Preprocessor Builder (reusable after feature engineering)
# ─────────────────────────────────────────────────────────────────────────────

def build_preprocessor(X: pd.DataFrame, config: PipelineConfig) -> ColumnTransformer:
    """Build a ColumnTransformer based on current column types in X."""
    num_cols = list(X.select_dtypes(include=[np.number]).columns)
    cat_cols = list(X.select_dtypes(include=["object", "category"]).columns)
...
def run_pipeline(data_path: str, target: Optional[str] = None, task: Optional[str] = None):
    ...
    print("\n╔══════════════════════════════════════════════════════════╗")
    print("║          AUTO-ML PIPELINE  v2.0 (Dual Mode)            ║")
    print("╚══════════════════════════════════════════════════════════╝")
    ...
    if task is None:
        ...
        if target is None:
            print("  [WARNING] --task both requires a --target. Falling back to clustering.")
            task = "clustering"
        else:
            print(f"  [User-specified] Task: BOTH (classification + regression)")
    else:
    ...
    # Rebuild preprocessor after feature engineering
    preprocessor = build_preprocessor(X, config)

    # ══════════════════════════════════════════════════════════════════════════
    # DUAL MODE: run both classification and regression
    # ══════════════════════════════════════════════════════════════════════════
    if task == "both":
        target_is_continuous = (
            pd.api.types.is_numeric_dtype(df[target])
            and profile["target_nunique"] > 20
        )

        if target_is_continuous:
            # ── REGRESSION (original continuous target) ──
            y_reg = y.copy()
            reg_results, reg_pipeline, reg_best = train_and_evaluate(
                X, y_reg, build_preprocessor(X, config), "regression", config, tag="original target"
            )
            reg_results.to_csv(os.path.join(output_dir, "regression_models.csv"), index=False)
            save_model_comparison_chart(reg_results, reg_best, "regression", output_dir, suffix="regression")
            reg_metrics = evaluate_best_model(
                reg_pipeline, X, y_reg, "regression", None, config, output_dir, suffix="regression"
            )
            save_feature_importance(reg_pipeline, X, output_dir, suffix="regression")

            # ── CLASSIFICATION (binned target) ──
            print("\n── CONVERTING TARGET TO CLASSES (quantile binning) ──")
            y_clf, bin_labels, bin_edges = bin_continuous_target(y, config.n_bins_for_classification)

            bin_le = LabelEncoder()
            bin_le.classes_ = np.array(bin_labels)

            clf_results, clf_pipeline, clf_best = train_and_evaluate(
                X, y_clf, build_preprocessor(X, config), "classification", config, tag="binned target"
            )
            clf_results.to_csv(os.path.join(output_dir, "classification_models.csv"), index=False)
            save_model_comparison_chart(clf_results, clf_best, "classification", output_dir, suffix="classification")
            clf_metrics = evaluate_best_model(
                clf_pipeline, X, y_clf, "classification", bin_le, config, output_dir, suffix="classification"
            )
            save_feature_importance(clf_pipeline, X, output_dir, suffix="classification")

        else:
            # ── CLASSIFICATION (original categorical / low-cardinality target) ──
            y_clf = y.copy()
            clf_results, clf_pipeline, clf_best = train_and_evaluate(
                X, y_clf, build_preprocessor(X, config), "classification", config, tag="original target"
            )
            clf_results.to_csv(os.path.join(output_dir, "classification_models.csv"), index=False)
            save_model_comparison_chart(clf_results, clf_best, "classification", output_dir, suffix="classification")
            clf_metrics = evaluate_best_model(
                clf_pipeline, X, y_clf, "classification", label_encoder, config, output_dir, suffix="classification"
            )
            save_feature_importance(clf_pipeline, X, output_dir, suffix="classification")

            # ── REGRESSION (use label-encoded numeric values) ──
            print("\n── CONVERTING TARGET TO NUMERIC (label encoding) ──")
            y_reg = encode_categorical_as_numeric(y, label_encoder)

            reg_results, reg_pipeline, reg_best = train_and_evaluate(
                X, y_reg, build_preprocessor(X, config), "regression", config, tag="encoded target"
            )
            reg_results.to_csv(os.path.join(output_dir, "regression_models.csv"), index=False)
            save_model_comparison_chart(reg_results, reg_best, "regression", output_dir, suffix="regression")
            reg_metrics = evaluate_best_model(
        ...
        generate_dual_report(
            ...
            clf_best, reg_best, output_dir,
        )

    # ══════════════════════════════════════════════════════════════════════════
    # SINGLE MODE: classification or regression only
    # ══════════════════════════════════════════════════════════════════════════
    elif task in ("classification", "regression"):
        results_df, best_pipeline, best_name = train_and_evaluate(
            X, y, preprocessor, task, config
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="AutoML Pipeline for Tabular Data (v2.0 — Dual Mode)")
    ...
    parser.add_argument("--task", default=None,
                        choices=["classification", "regression", "clustering", "both"],
                        help="Task type: auto-detected if omitted, or 'both' to run classification + regression simultaneously")
